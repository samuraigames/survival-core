...
const EngineRepairMinigame: React.FC<EngineRepairMinigameProps> = ({ open, onClose, difficulty }) => {
  ...
  const handleMouseUp = (e: React.MouseEvent) => {
    ...
    for (let i = 0; i < numWires; i++) {
        ...
        if (distance < 20) { // 20 is the radius of the node
            ...
            const endColorIndex = wirePuzzle.ends[i];

            if (startColorIndex === endColorIndex) {
                const newConnections = [...connections];
                newConnections[currentDrag.from] = i;
                setConnections(newConnections);
  ...
  const Node = ({ index, side, puzzle }: { index: number; side: 'left' | 'right', puzzle: WirePuzzle }) => {
      ...
      <DialogContent className="max-w-md bg-card border-accent text-foreground">
        ...
        <div className="relative w-full h-96">
          ...
          <svg ref={svgRef} className="w-full h-80" onMouseMove={handleMouseMove} onMouseUp={handleMouseUp}>
            ...
            {wirePuzzle && connections.map((endIndex, startIndex) => {
